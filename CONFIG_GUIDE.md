...
BOOTSTRAP_SERVERS=broker1:9092,broker2:9092 ./producer-benchmark-runner.sh
```

### Method 2: Configuration File (TLS/Complex Scenarios)
```bash
# Use producer.config file
CLIENT_CONFIG=producer.config ./producer-benchmark-runner.sh

# With custom number of records
CLIENT_CONFIG=producer.config NUM_RECORDS=5000000 ./producer-benchmark-runner.sh

# With custom config file path
CLIENT_CONFIG=/path/to/my-producer.properties ./producer-benchmark-runner.sh
```
...
CLIENT_CONFIG=producer-high-throughput.config ./producer-benchmark-runner.sh

# Low latency test
CLIENT_CONFIG=producer-low-latency.config ./producer-benchmark-runner.sh
...
4. **Test with small NUM_RECORDS first** when using new configurations:
   ```bash
   CLIENT_CONFIG=producer.config NUM_RECORDS=10000 ./producer-benchmark-runner.sh
   ```
...
CLIENT_CONFIG=producer-tls.config NUM_RECORDS=1000000 ./producer-benchmark-runner.sh
